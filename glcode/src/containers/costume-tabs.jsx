...
import AssetPanels from "../components/asset-panels/asset-panels.jsx";
...
class CostumeTabs extends React.Component {
    constructor(props) {
        super(props);
        bindAll(this, [
            "handleSelectCostume",
        ...
        console.log("isFromAddPaintss", props.isFromAddPaint);
        console.log(target.currentCostume);
        if (target && target.currentCostume) {
            this.state = {
                selectedCostumeIndex: target.currentCostume,
                editedItemIsOpen: false,
            };
        } else if (
            target &&
            target.currentCostume === 0 &&
            props.isFromAddPaint === true
        ) {
            this.state = {
                selectedCostumeIndex: target.currentCostume,
                editedItemIsOpen: true,
            };
        } else {
            this.state = { selectedCostumeIndex: 0, editedItemIsOpen: false };
        }
    }
    componentDidMount() {
        var that = this;
    ...
    componentWillReceiveProps(nextProps) {
        const { editingTarget, sprites, stage, isFromAddPaint } = nextProps;
        const target =
            editingTarget && sprites[editingTarget]
                ? sprites[editingTarget]
                : stage;
    ...
    handleNewBlankCostume() {
        console.log("handleNewBlankCostume");
    ...
    handleSurpriseCostume() {
        const item =
            this.state.costumeLibraryContent[
                Math.floor(
                    Math.random() * this.state.costumeLibraryContent.length
                )
            ];
        const vmCostume = {
            name: item.name,
    ...
    handleOpen = (v) => {
        ...
        this.setState({ editedItemIsOpen: true }, () => {
            console.log("checkeditedItemIsOpen", this.state.editedItemIsOpen);
        });
    };
    handleClose = (v) => {
        console.log("handleClose", v);
        this.setState({ editedItemIsOpen: false }, () => {
            console.log("checkeditedItemIsOpen", this.state.editedItemIsOpen);
        });
    };
    setFileInput(input) {
    ...
    render() {
        ...
        const costumeData = target.costumes
            ...
            : [];
        console.log("costumeData", costumeData);
        return (
            <AssetPanels
                buttons={[
                    {
                ...
                editedItemIsOpen={this.state.editedItemIsOpen}
                onDeleteClick={
...
export default errorBoundaryHOC("Costume Tab")(
    injectIntl(connect(mapStateToProps, mapDispatchToProps)(CostumeTabs))
);
