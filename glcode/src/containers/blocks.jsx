...
import srarchSvg from "./search.svg";
...
class Blocks extends React.Component {
    constructor(props) {
        ...
        this.state = {
            prompt: null,
            //  zoomScale: BLOCKS_DEFAULT_SCALE * 100 + "%",
        };
    ...
    crearSearchBox() {
        //这是生成搜索图标的
        let dom = document.getElementsByClassName("blocklyToolboxDiv");

        let srarch = document.createElement("div");
        let searchIMg = document.createElement("img");
        dom[0].appendChild(srarch);
        srarch.setAttribute("class", "searchIcon");

        srarch.style.setProperty("width", "60px");
        srarch.style.setProperty("height", "40px");

        srarch.appendChild(searchIMg);

        searchIMg.style.setProperty("width", "30px");
        searchIMg.style.setProperty("height", "30px");
        searchIMg.style.setProperty("margin-left", "15px");
        searchIMg.style.setProperty("margin-top", "5px");
        searchIMg.src = srarchSvg;

        //搜索框
        let Olddom = document.getElementsByClassName("blocklyFlyoutBackground");

        let boxDom = document.createElement("div");
        Olddom[0].appendChild(boxDom);
        boxDom;
        boxDom.style.setProperty("width", "100%");
        boxDom.style.setProperty("height", "80px");
        boxDom.setAttribute("class", "red");
        boxDom.style.setProperty("background-color", "red");

        var that = this;

        searchIMg.onclick = function () {
            that.props.onsearchKey();
        };
    }
    ...
    getToolboxXML() {
        ...
        try {
            ...
            //这里还没编译
            // console.log(makeToolboxXML(false, target.isStage, target.id, dynamicBlocksXML,
    ...
    //     this.setState(
    //         {
    //             zoomScale: this.ceilByNum(v * 100, 5),
    //         },
    //         () => {
    //             document.getElementById('zoomScale').innerHTML = this.ceilByNum(this.state.zoomScale,5)+'%';
    //         }
    //     );
    // };
    // ceilByNum(num, N) {
    //     var rem = num % N;
    //     var quo = Math.floor(num / N);
    //     if (rem > N / 2) {
    //         return quo * N + N;
    //     } else {
    //         return quo * N;
